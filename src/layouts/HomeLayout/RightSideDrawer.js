...
const NavBar = () => {
  ...
  const handleDesconnect = () => {
    let checkWallet = sessionStorage.getItem("isanotherWallet")
    if (checkWallet) {
    user?.disconnectAnotherWallet();
    setRightBar(false);
  ...
  return (
    <Box display="flex" alignItems="center">
      ...
      {CoinName?.coinImage &&
        <img src={CoinName?.coinImage} alt="coin" style={{width:"30px"}} />
      }
      <Typography style={{
        marginLeft: "5px",
        color: "#fff",
      ...
      {account  || user?.accounts? (
        ...
        >
          {/* {account} */}
          {user?.accounts?sortAddress(user?.accounts):sortAddress(account)}
          <Tooltip title="Copy">
