...
export default function AuthProvider(props) {
  ...
  const getSubscription = async () => {
    try {
      ...
      if (res.status === 200) {
        setSubsritionList(res.data.result.filter((data) => data.isDefaultJava !== false && data.isDefaultJava !== true))
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (window.sessionStorage.getItem('walletName')) {
      const selectectWalletDetails = SUPPORTED_WALLETS.filter((data) => data.name === window.sessionStorage.getItem('walletName'))
  ...
  const getStakingData = async () => {
    try {
      ...
      if (res.data.status === 200) {
        ...
        setSetalite(res.data.data.filter((data) => data.stakingName === 'SATELLITE'))
        setTaral(res.data.data.filter((data) => data.stakingName === 'TARALITY'))
        setRodmars(res.data.data.filter((data) => data.stakingName === 'REDMARS'))
        setREvolution(res.data.data.filter((data) => data.stakingName === 'REVOLUTION'))
      } else {
      }
      // setIsLoadingData(false);
    } catch (error) {
